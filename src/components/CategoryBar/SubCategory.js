...
function SubCategory({ subcategory }) {
  let icon =
    'https://yanxuan.nosdn.127.net/6eeaa813566307004ff88f1013bf08fe.png';

  let thirdCategoryNodes = null;
  if (subcategory && !_.isEmpty(subcategory.thirdCategories)) {
    thirdCategoryNodes = subcategory.thirdCategories.map(thirdCat => (
      <CategoryItem
        key={thirdCat.id}
        icon={thirdCat.icon || icon}
        title={thirdCat.title}
      />
    ));
  }
  ...
  return (
    <div className={$style.subcategory}>
      <div className={classnames($style.subcategory__title)}>
        {subcategory.title}
      </div>
